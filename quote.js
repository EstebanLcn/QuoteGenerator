...
let rdmNumber = 0; // 
  ...
  function createRdmNumber(max) {
    return Math.floor(Math.random() * Math.floor(max));
  }
  
  newQuote.addEventListener('click', () => {
      console.log(quotes[2][0]);
    do {
        rdmNumber = createRdmNumber(quotes.length);
     } while (rdmNumber == last);

     quoteDiv.textContent = quotes[rdmNumber][0];
     authorDiv.textContent = quotes[rdmNumber][1];
     last = rdmNumber;
  });
